...
export function setScopes(_scopes){
    scopes = _scopes;
}
...
export function setBytes(_bytes: Uint8Array){
    bytes = _bytes
}
...
export class Block{
    ...
    runChild(blockId){
        if(scopes[blockId][1] !== this.blockId) throw(`${blockId} is not a child scope of ${this.blockId}`);
